...
{
	slideright("list_of_history","profile_screen");
...
$("#history_back").on("click", function(){
	sliderright("list_of_history_screen", "profile_screen");
  	setTimeout(function(){
    	$("#list_of_history_screen").empty();
    }, 500);
});
  	...
  	this.openDB = function(){
    	this.db = openDatabase("History", "1.0", "Items", 2*1024*1024);
     	this.db.transaction(function(tx){
        	tx.executeSql("CREATE TABLE IF NOT EXISTS history (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, now, create, title, notes)");
        });
    };
      	...
      	function querySuccess(tx, results){
        	var len = results.rows.length;
          	if(len > 0){
            	var htmlArray = [];
              	for (var i=0; i<len; i++){
                	var html = "<li id='" + results.rows.item(i).now + "' class='history_li'><span class='delete_item'>x</span><span class='h_title'>" + results.rows.item(i).title + "</span><span class='h_notes'>" + results.rows.item(i).notes + "</span></li>";
